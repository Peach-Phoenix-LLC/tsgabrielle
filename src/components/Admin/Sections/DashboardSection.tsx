    ...
    return (
        <div className="space-y-12 bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { label: 'Revenue', value: stats?.revenue || '$0.00', icon: 'payments', trend: '+12%' },
                    { label: 'Orders', value: stats?.orders || '0', icon: 'local_shipping', trend: '+5%' },
                    { label: 'AOV', value: stats?.aov || '$0.00', icon: 'analytics', trend: 'Stable' },
                    { label: 'Products', value: stats?.products || '0', icon: 'inventory_2', trend: 'Active' },
                ].map((stat, i) => (
                    <div key={i} className="bg-neutral-50 border border-black/5 p-6 rounded-2xl hover:bg-neutral-100 transition-all group shadow-sm">
                        ...
                        <p className={`text-[10px] mt-4 ${stat.trend.startsWith('+') ? 'text-green-600' : 'text-[#1a1a1a]/20'}`}>{stat.trend}</p>
                    </div>
                ))}
            </div>
