...
export default async function ProductPage({ params }: PageProps) {
    ...
    // Validate that peach is an integer
    if (!/^\d+$/.test(peach)) {
        notFound();
    }
    ...
    return (
        <main className="bg-black text-white min-h-screen selection:bg-[#a932bd] selection:text-white">
            ...
            {/* Footer Spacer */}
            <div className="h-40 border-t border-white/5 bg-gradient-to-t from-zinc-950 to-black flex items-center justify-center">
                <p className="text-[10px] uppercase tracking-[0.5em] opacity-20">Paris • Phoenix • tsgabrielle®</p>
            </div>
        </main>
    );
}
