...
export async function POST(req: NextRequest) {
    ...
    try {
        let result;
        const model: any = type === 'categories' ? prisma.category : type === 'collections' ? prisma.collection : prisma.cMSPage;

        if (action === 'create') result = await model.create({ data });
        else if (action === 'update') result = await model.update({ where: { id }, data });
        else if (action === 'delete') result = await model.delete({ where: { id } });
