...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!collection) {
    return buildMetadata({ title: "Collection Not Found", description: "" });
  }

  const isParis = resolvedParams.slug === "paris";
  const seoTitle = isParis
    ? "Paris Collection • tsgabrielle®"
    : collection.seo_title || `${collection.name} | tsgabrielle`;
  const seoDesc = isParis
    ? "Discover Paris by tsgabrielle® — a luxury streetwear collection blending French elegance, identity, and modern design. The French Trans Touch™ at its finest."
    : collection.seo_description || collection.description || "Explore this exclusive collection at tsgabrielle.";
