...
export default function CollectionSection() {
  ...
  function startCreate() {
    setEditingItem({} as CollectionItem);
  ...
  return (
    <div className="space-y-8">
      <header className="flex justify-between items-center">
        <h3 className="text-xl font-light">Curated Collections</h3>
        {!editingItem && (
          <button
            onClick={startCreate}
      ...
      {editingItem && (
        <form onSubmit={handleSubmit} className="bg-[#fdfcf5] p-8 rounded-2xl border border-black/5 space-y-6">
          ...
          <Field
            label="Tags (max 25, comma separated)"
            value={formData.tagsInput}
            onChange={(value) => setFormData({ ...formData, tagsInput: value })}
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <TextAreaField
              ...
              value={formData.hero_description_1}
              onChange={(value) => setFormData({ ...formData, hero_description_1: value })}
            />
            <TextAreaField
              label="Hero Description 2"
              value={formData.hero_description_2}
              onChange={(value) => setFormData({ ...formData, hero_description_2: value })}
            />
            <TextAreaField
              label="Hero Description 3"
              value={formData.hero_description_3}
              onChange={(value) => setFormData({ ...formData, hero_description_3: value })}
            />
          </div>

          <div>
            <p className="text-[10px] uppercase font-bold mb-2">Collection Colors</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Field
                label="Product Grid Background"
                value={formData.product_grid_background_color}
                onChange={(value) => setFormData({ ...formData, product_grid_background_color: value })}
              />
              <Field
                label="Product Grid Text"
                value={formData.product_grid_text_color}
                onChange={(value) => setFormData({ ...formData, product_grid_text_color: value })}
              />
              <Field
                label="Product Grid Accent"
                value={formData.product_grid_accent_color}
                onChange={(value) => setFormData({ ...formData, product_grid_accent_color: value })}
              />
            </div>
          </div>
